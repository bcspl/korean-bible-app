...
### 현재 코드 상태
- `lib/main.dart`: Hive init + 기본 HomeScreen (placeholder UI)
- 데이터 구조: 간단한 JSON (`version`, `books[]` → `chapters[]` → `verses[]`)
- 아직 구현되지 않은 핵심 기능: 데이터 로딩, 실제 성경 뷰, Hive 모델, 검색 등

### 문서 상태
- 본 로드맵: 작성 완료 (이 문서)
- `Task_Prioritized_Backlog_and_Schedule_2026-06-19.md`: Placeholder
